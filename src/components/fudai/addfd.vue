      ...
      <div class="addimg">
        <div class="addimg1">
          ...
          <div style="width: 2.1rem;height: 1.6rem;position: relative;">
            ...
            <img :src="modelSrc" alt="" style="width: 2.1rem;height: 1.6rem;">
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      jobdone () {
        console.log(this.fudaiInfo.file)
        if (!this.fudaiInfo.describe) {
          this.$toast('请填写消费须知')
        } else if (!this.fudaiInfo.title) {
          this.$toast('请填写消费须知')
        } else {
          this.$http.post('/store/v1/wallets', this.fudaiInfo).then(
            res => {
              console.log(res)
            }
